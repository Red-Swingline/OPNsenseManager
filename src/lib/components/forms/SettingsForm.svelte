<script lang="ts">
  ...
  const dispatch = createEventDispatcher<{
    ...
    submit: { profileName: string; apiKey: string; apiSecret: string; apiUrl: string; port: number; pin: string };
  ...
  async function loadProfileInfo(profileName: string): Promise<void> {
    try {
      const profileInfo = await invoke<ApiInfo>('get_api_info', { profileName });
      if (profileInfo) {
  ...
  <form on:submit|preventDefault={handleSubmit} class="space-y-6">
    <div class="form-control">
      ...
      <div class="flex space-x-2">
        ...
        <button type="button" class="btn btn-primary" on:click={openAddProfileModal}>
          Add Profile
        </button>
        {#if profiles.length > 1}
          <button type="button" class="btn btn-error" on:click={openDeleteConfirmation}>
            Delete
          </button>
        {/if}
      </div>
  ...
  {#if showDeleteConfirmation}
    <div class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
      <div class="bg-base-100 p-6 rounded-lg max-w-sm w-full">
        ...
        <p class="mb-4">Are you sure you want to delete the profile "{selectedProfileName}"?</p>
      ...
      <div class="bg-base-100 p-6 rounded-lg max-w-md w-full">
        ...
        <form on:submit|preventDefault={addNewProfile} class="space-y-4">
          <div class="form-control">
            ...
            <input id="newProfileName" bind:value={newProfileName} type="text" placeholder="Enter profile name" class="input input-bordered w-full" required />
          </div>
          <div class="form-control">
            <label class="label" for="newApiKey">
              <span class="label-text">API Key</span>
            </label>
            <input id="newApiKey" bind:value={newApiKey} type="text" placeholder="Enter API Key" class="input input-bordered w-full" required />
          </div>
          <div class="form-control">
            <label class="label" for="newApiSecret">
              <span class="label-text">API Secret</span>
            </label>
            <input id="newApiSecret" bind:value={newApiSecret} type="password" placeholder="Enter API Secret" class="input input-bordered w-full" required />
          </div>
          <div class="form-control">
            <label class="label" for="newApiUrl">
              <span class="label-text">API URL</span>
            </label>
            <input id="newApiUrl" bind:value={newApiUrl} type="url" placeholder="Enter API URL" class="input input-bordered w-full" required />
          </div>
          <div class="form-control">
            <label class="label" for="newPort">
              <span class="label-text">Port</span>
            </label>
            <input id="newPort" bind:value={newPort} type="number" placeholder="Enter Port" class="input input-bordered w-full" required />
          </div>
          <div class="flex justify-end space-x-2 mt-6">
            <button type="button" class="btn btn-ghost" on:click={closeAddProfileModal}>Cancel</button>
